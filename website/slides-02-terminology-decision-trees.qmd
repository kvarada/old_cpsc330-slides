...
model = DummyClassifier(strategy="most_frequent") # Define a class object of the model 
model.fit(X, y)
...
- What would be the prediction for the example below using the tree above? 
  - supportive_colleagues = 0, hours_flexibility = 0, start_up =1, coffee_machine = 1, vegan_boss = 1,  
```{python}
plot_tree(model, filled=True, feature_names = X.columns, impurity = False, fontsize=16);
```
...
- Parameters 
  ...
  - Example: supportive_colleagues <= 0.5 at the root node  
- Hyperparameters
  - Settings that control tree growth, like `max_depth`, which limits how deep the tree can go.

## Decision boundary with `max_depth=1`
```{python}
X_subset = X[["supportive_colleagues", "coffee_machine"]]
model = DecisionTreeClassifier(max_depth=1)
model.fit(X_subset.values, y)
plot_tree_decision_boundary_and_tree(
    model, X_subset, y, x_label="supportive_colleagues", y_label="coffee_machine", fontsize=12, class_names = ["Happy", "Unhappy"]
)
```
